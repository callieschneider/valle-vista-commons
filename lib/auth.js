...
async function requireMod(req, res, next) {
  ...
  if (SUPER_PASS && creds.user === SUPER_USER && creds.pass === SUPER_PASS) {
    ...
    req.authUser = SUPER_USER; // For audit logging
    req.modId = null; // Super admin is not a mod
    return next();
  }
  ...
  try {
    ...
    req.mod = mod; // Attach mod object to request
    req.modId = mod.id;
    req.authUser = mod.username; // For audit logging
    next();
  } catch (err) {
